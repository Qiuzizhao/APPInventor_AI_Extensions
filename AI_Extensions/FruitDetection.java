...
@DesignerComponent(version = 1, description = "Fruit Detection", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class FruitDetection extends AIXBase {
    ...
    public FruitDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //检测水果
    @SimpleFunction(description = "DetectFruit")
    public void DetectFruit(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloFruit
            params.put("ai_type", "YoloFruit");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    FruitDetection.this.results = response.getJSONObject("data");
                    FruitDetection.this.resultBase64 = FruitDetection.this.results.getString("image");
                    FruitDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    FruitDetection.this.raiseError("FruitDetection.DetectFruit", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("FruitDetection.DetectFruit", -1, e.toString());
        }
    }
