...
@DesignerComponent(version = 1, description = "Dog Leash Detection", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class DogLeashDetection extends AIXBase {
    ...
    public DogLeashDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //检测狗绳
    @SimpleFunction(description = "DetectDogLeash")
    public void DetectDogLeash(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloDogLeash
            params.put("ai_type", "YoloDogLeash");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    DogLeashDetection.this.results = response.getJSONObject("data");
                    DogLeashDetection.this.resultBase64 = DogLeashDetection.this.results.getString("image");
                    DogLeashDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    DogLeashDetection.this.raiseError("DogLeashDetection.DetectDogLeash", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("DogLeashDetection.DetectDogLeash", -1, e.toString());
        }
    }
