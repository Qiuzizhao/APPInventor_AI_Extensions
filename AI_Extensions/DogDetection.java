...
@DesignerComponent(version = 1, description = "Dog Detection", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class DogDetection extends AIXBase {
    ...
    public DogDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //检测狗
    @SimpleFunction(description = "DetectDog")
    public void DetectDog(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloDog
            params.put("ai_type", "YoloDog");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    DogDetection.this.results = response.getJSONObject("data");
                    DogDetection.this.resultBase64 = DogDetection.this.results.getString("image");
                    DogDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    DogDetection.this.raiseError("DogDetection.DetectDog", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("DogDetection.DetectDog", -1, e.toString());
        }
    }
