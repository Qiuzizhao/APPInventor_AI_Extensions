...
@DesignerComponent(version = 1, description = "Phone Detection", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class PhoneDetection extends AIXBase{
    ...
    public PhoneDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //检测手机
    @SimpleFunction(description = "DetectPhone")
    public void DetectPhone(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloPhone
            params.put("ai_type", "YoloPhone");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    PhoneDetection.this.results = response.getJSONObject("data");
                    PhoneDetection.this.resultBase64 = PhoneDetection.this.results.getString("image");
                    PhoneDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    PhoneDetection.this.raiseError("PhoneDetection.DetectPhone", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("PhoneDetection.DetectPhone", -1, e.toString());
        }
    }
