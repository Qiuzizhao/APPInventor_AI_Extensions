...
@DesignerComponent(version = 1, description = "Garbage Detection", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class GarbageDetection extends AIXBase{
    ...
    public GarbageDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //检测垃圾
    @SimpleFunction(description = "DetectGarbage")
    public void DetectGarbage(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloGarbage
            params.put("ai_type", "YoloGarbage");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    GarbageDetection.this.results = response.getJSONObject("data");
                    GarbageDetection.this.resultBase64 = GarbageDetection.this.results.getString("image");
                    GarbageDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    GarbageDetection.this.raiseError("GarbageDetection.DetectGarbage", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("GarbageDetection.DetectGarbage", -1, e.toString());
        }
    }
