...
@DesignerComponent(version = 1, description = "Garbage Sort", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class GarbageSortDetection extends AIXBase{
    ...
    public GarbageSortDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //垃圾分类
    @SimpleFunction(description = "SortGarbage")
    public void SortGarbage(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloGarbageSort
            params.put("ai_type", "YoloGarbageSort");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    GarbageSortDetection.this.results = response.getJSONObject("data");
                    GarbageSortDetection.this.resultBase64 = GarbageSortDetection.this.results.getString("image");
                    GarbageSortDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    GarbageSortDetection.this.raiseError("GarbageSortDetection.SortGarbage", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("GarbageSortDetection.SortGarbage", -1, e.toString());
        }
    }
