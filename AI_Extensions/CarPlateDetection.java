...
@DesignerComponent(version = 1, description = "Car Plate Detection", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class CarPlateDetection extends AIXBase {
    ...
    public CarPlateDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //检测车牌
    @SimpleFunction(description = "DetectCarPlate")
    public void DetectCarPlate(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloCarPlate
            params.put("ai_type", "YoloCarPlate");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    CarPlateDetection.this.results = response.getJSONObject("data");
                    CarPlateDetection.this.resultBase64 = CarPlateDetection.this.results.getString("image");
                    CarPlateDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    CarPlateDetection.this.raiseError("CarPlateDetection.DetectCarPlate", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("CarPlateDetection.DetectCarPlate", -1, e.toString());
        }
    }
