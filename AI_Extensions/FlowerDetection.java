...
@DesignerComponent(version = 1, description = "Flower Detection", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class FlowerDetection extends AIXBase{
    ...
    public FlowerDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //检测花朵
    @SimpleFunction(description = "DetectFlower")
    public void DetectFlower(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloFlower
            params.put("ai_type", "YoloFlower");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    FlowerDetection.this.results = response.getJSONObject("data");
                    FlowerDetection.this.resultBase64 = FlowerDetection.this.results.getString("image");
                    FlowerDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    FlowerDetection.this.raiseError("FlowerDetection.DetectFlower", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("FlowerDetection.DetectFlower", -1, e.toString());
        }
    }
