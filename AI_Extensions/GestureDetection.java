...
@DesignerComponent(version = 1, description = "Gesture Detection", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class GestureDetection extends AIXBase{
    ...
    public GestureDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //检测手势
    @SimpleFunction(description = "DetectGesture")
    public void DetectGesture(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloGesture
            params.put("ai_type", "YoloGesture");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    GestureDetection.this.results = response.getJSONObject("data");
                    GestureDetection.this.resultBase64 = GestureDetection.this.results.getString("image");
                    GestureDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    GestureDetection.this.raiseError("GestureDetection.DetectGesture", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("GestureDetection.DetectGesture", -1, e.toString());
        }
    }
