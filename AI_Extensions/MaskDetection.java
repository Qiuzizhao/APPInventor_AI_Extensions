...
@DesignerComponent(version = 1, description = "Mask Detection", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class MaskDetection extends AIXBase{
    ...
    public MaskDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //检测口罩
    @SimpleFunction(description = "DetectMask")
    public void DetectMask(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloMask
            params.put("ai_type", "YoloMask");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    MaskDetection.this.results = response.getJSONObject("data");
                    MaskDetection.this.resultBase64 = MaskDetection.this.results.getString("image");
                    MaskDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    MaskDetection.this.raiseError("MaskDetection.DetectMask", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("MaskDetection.DetectMask", -1, e.toString());
        }
    }
