...
@DesignerComponent(version = 1, description = "Smoke Detection", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class SmokeDetection extends AIXBase {
    ...
    public SmokeDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //检测抽烟
    @SimpleFunction(description = "DetectSmoke")
    public void DetectSmoke(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloSmoke
            params.put("ai_type", "YoloSmoke");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    SmokeDetection.this.results = response.getJSONObject("data");
                    SmokeDetection.this.resultBase64 = SmokeDetection.this.results.getString("image");
                    SmokeDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    SmokeDetection.this.raiseError("SmokeDetection.DetectSmoke", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("SmokeDetection.DetectSmoke", -1, e.toString());
        }
    }
