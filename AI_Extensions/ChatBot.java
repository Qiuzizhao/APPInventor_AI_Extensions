...
@DesignerComponent(version = 1, description = "ChatBot", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class ChatBot extends AIXBase {
    private static final String PATH = "/chatbot";
    ...
    public void Ask(String question, boolean isKnowledge) {
        try {
            //键值对
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    ChatBot.this.answer = response.getString("data");
                    ChatBot.this.GotResult();
                }

                public void onFailure(String message) {
                    ChatBot.this.raiseError("ChatBot.Ask", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("ChatBot.Ask", -1, e.toString());
        }
    }
