...
@DesignerComponent(version = 1, description = "Animal Detection", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class AnimalDetection extends AIXBase {
    ...
    public AnimalDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //检测动物
    @SimpleFunction(description = "DetectAnimal")
    public void DetectAnimal(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloAnimal
            params.put("ai_type", "YoloAnimal");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    AnimalDetection.this.results = response.getJSONObject("data");
                    AnimalDetection.this.resultBase64 = AnimalDetection.this.results.getString("image");
                    AnimalDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    AnimalDetection.this.raiseError("AnimalDetection.DetectAnimal", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("AnimalDetection.DetectAnimal", -1, e.toString());
        }
    }
