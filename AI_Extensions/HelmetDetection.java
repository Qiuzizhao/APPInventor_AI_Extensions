...
@DesignerComponent(version = 1, description = "Helmet Detection", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class HelmetDetection extends AIXBase {
    ...
    public HelmetDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //检测安全帽
    @SimpleFunction(description = "DetectHelmet")
    public void DetectHelmet(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloHelmet
            params.put("ai_type", "YoloHelmet");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    HelmetDetection.this.results = response.getJSONObject("data");
                    HelmetDetection.this.resultBase64 = HelmetDetection.this.results.getString("image");
                    HelmetDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    HelmetDetection.this.raiseError("HelmetDetection.DetectHelmet", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("HelmetDetection.DetectHelmet", -1, e.toString());
        }
    }
