...
@DesignerComponent(version = 1,
    description = "Music Generation component that generates audio via server",
...
public class MusicGen extends AIXBase implements OnDestroyListener {

    private static final String PATH = "/musicgen";
    ...
    public MusicGen(ComponentContainer container) {
        super(container);
    ...
    // Generate function
    @SimpleFunction(description = "Generates music from the given prompt")
    public void Generate(String prompt) {
        try {
            BeforeGenerating();
            
            Map<String, Object> params = new HashMap<>();
            params.put("operation", "musicgen");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                ...
                public void onFailure(String message) {
                    AfterGenerating(false);
                    raiseError("MusicGen.Generate", -1, message);
                }
            });
        } catch (Exception e) {
            AfterGenerating(false);
            raiseError("MusicGen.Generate", -1, e.toString());
        }
    }

    // Stop function
    @SimpleFunction(description = "Stops the current music")
    public void Stop() {
    ...
    // Play function
    @SimpleFunction(description = "play the current music")
    public void playAudio() {
        try {
            // Convert base64 to temp file
            byte[] audioData = Base64.decode(audioBase64, Base64.DEFAULT);
            File tempFile = File.createTempFile("music_gen", ".mp3", form.getCacheDir());
            ...
            AfterGenerating(false);
            raiseError("MusicGen.playAudio", -1, e.toString());
        }
    }

    @SimpleEvent(description = "Event raised before generating starts")
    public void BeforeGenerating() {
        EventDispatcher.dispatchEvent(this, "BeforeGenerating");
    }

    @SimpleEvent(description = "Event raised after generating ends")
    public void AfterGenerating(boolean success) {
        EventDispatcher.dispatchEvent(this, "AfterGenerating", success);
    }
