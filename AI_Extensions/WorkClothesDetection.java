...
@DesignerComponent(version = 1, description = "Work Clothes Detection", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class WorkClothesDetection extends AIXBase {
    ...
    public WorkClothesDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //检测工作服
    @SimpleFunction(description = "DetectWorkClothes")
    public void DetectWorkClothes(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloWorkClothes
            params.put("ai_type", "YoloWorkClothes");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    WorkClothesDetection.this.results = response.getJSONObject("data");
                    WorkClothesDetection.this.resultBase64 = WorkClothesDetection.this.results.getString("image");
                    WorkClothesDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    WorkClothesDetection.this.raiseError("WorkClothesDetection.DetectWorkClothes", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("WorkClothesDetection.DetectWorkClothes", -1, e.toString());
        }
    }
