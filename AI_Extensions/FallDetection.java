...
@DesignerComponent(version = 1, description = "Fall Detection", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class FallDetection extends AIXBase{
    ...
    public FallDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //检测跌倒
    @SimpleFunction(description = "DetectFall")
    public void DetectFall(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloFall
            params.put("ai_type", "YoloFall");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    FallDetection.this.results = response.getJSONObject("data");
                    FallDetection.this.resultBase64 = FallDetection.this.results.getString("image");
                    FallDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    FallDetection.this.raiseError("FallDetection.DetectFall", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("FallDetection.DetectFall", -1, e.toString());
        }
    }
