...
@DesignerComponent(version = 1, description = "Fire Detection", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "images/extension.png")
@UsesPermissions(permissionNames = "android.permission.INTERNET")
@UsesLibraries(libraries = "json.jar")
@SimpleObject(external = true)

public class FireDetection extends AIXBase{
    ...
    public FireDetection(ComponentContainer container) {
        super(container);
    }
    ...
    //检测火灾
    @SimpleFunction(description = "DetectFire")
    public void DetectFire(String inputImage) {
        try {
            Map<String, Object> params = new HashMap<>();
            
            // 设置AI类型为YoloFire
            params.put("ai_type", "YoloFire");
            ...
            post(PATH, params, new AsyncCallbackPair<JSONObject>() {
                public void onSuccess(JSONObject response) {
                    FireDetection.this.results = response.getJSONObject("data");
                    FireDetection.this.resultBase64 = FireDetection.this.results.getString("image");
                    FireDetection.this.GotResult();
                }

                public void onFailure(String message) {
                    FireDetection.this.raiseError("FireDetection.DetectFire", -1, message);
                }
            });
        } catch (Exception e) {
            raiseError("FireDetection.DetectFire", -1, e.toString());
        }
    }
